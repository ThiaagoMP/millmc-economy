...
class TradeAcceptCommand(private val messagesConfig: FileConfiguration, private val tradeController: TradeController) {

    @Command(name = "tradeaccept", target = CommandTarget.PLAYER)
    fun handleCommand(context: Context<CommandSender>) {
        ...
        if (targetPlayer == null || !targetPlayer.isOnline || !tradeController.tradesRequests.containsKey(targetPlayer)
            ...
            player.sendMessage(LanguageAPI.getTranslatedMessage("PLAYER_NOT_FOUND_TRADE", player, emptyMap(), messagesConfig))
            tradeController.tradesRequests.remove(targetPlayer)
            return
        }
        tradeController.tradesRequests.remove(targetPlayer)
        tradeController.startTrade(targetPlayer, player)
    }
